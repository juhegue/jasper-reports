    ...
    def generateIds(self, record, relations, path, currentRecords):
        ...
        for relation in unrepeated:
            ...
            if path:
                ...
                currentPath = root
            if root == 'Attachments':
                ids = self.pool['ir.attachment'].search(self.cr, self.uid, [
                    ('res_model', '=', record._name),
                    ('res_id', '=', record.id)
                ], context=self._context)
                value = self.pool['ir.attachment'].browse(
                    self.cr, self.uid, ids, context=self._context)
            elif root == 'User':
                value = self.pool['res.users'].browse(
                    self.cr, self.uid, [self.uid], context=self._context)
            else:
                if root == 'id':
                    value = record.id
